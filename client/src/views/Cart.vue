<template>
  <div>
        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li v-for="(item, index) in cartList" :key="index" v-if="!isCartNull">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascript:;" :class="{ 'check': item.checked==='1', 'checkbox-btn': true, 'item-check-btn': true }" @click="editCart('checked', item)">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a href="javascript:;" class="input-sub" @click="editCart('minu', item)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a href="javascript:;" class="input-add" @click="editCart('add', item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="delCartConfirm(item)" id="delBtn">
                      <svg>
              ...
              <li v-if="isCartNull">{{ nullCartTxt }}</li>
            </ul>
          </div>

          <div class="cart-foot-wrap" v-if="!isCartNull">
            <div class="cart-foot-inner">
              <div class="cart-foot-l">
                <div class="item-all-check">
                  <a href="javascript:;"  @click="toggleCheckAll()" >
                    <span :class="[ { 'check': isCheckedAll }, 'checkbox-btn', 'item-check-btn' ]">
                      <svg class="icon icon-ok">
                        <use xlink:href="#icon-ok"></use>
                      </svg>
                    </span>
    ...
    <modal :mdShow="modalConfirm">
      <div slot="message">你确定要删除这个商品吗？</div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      nullCartTxt() {
        return this.isCartNull ? '购物车空空如也！' : '';
      },
      checkedCount() {
        let i = 0;

        this.cartList.forEach(item=>{ if(item.checked === '1') i++; });
        return i;
      },
      isCheckedAll() {
      ...
      totalPrice() {
        ...
        this.cartList.forEach(item=>{ if(item.checked === '1') money += item.salePrice * item.productNum; });
        return money;
      }
    },
    mounted() {
    ...
    methods: {
      ...
      editCart(flag, item) {
        if (flag === 'minu' && item.productNum === 1) return alert('不能再减了，再减不如删除商品吧！');

        if (flag === 'checked') item.checked = (item.checked === '1' ? '0' : '1');
          ...
          if (res.status === 200) {
            switch (flag) {
              ...
              case 'add':
                ...
                break;
              default:
                console.log(item.productName + '商品已切换选中状态');
                break;
            }
          } else alert(res.msg);
        });
      ...
      toggleCheckAll() {
        ...
        this.cartList.forEach(item=>{ item.checked = flag ? '1' : '0'; });
        this.$http.post('/users/editCheckAll',{ checkAllVal: this.cartList[0].checked }).then(result=>{
          let res = result.data;
          console.log(res.msg);
          if(res.status !== 200) alert(res.msg);
        })
      },
      checkout() {
