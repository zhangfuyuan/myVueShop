      ...
      <div class="container">
        <div class="checkout-addr">
            ...
            <div class="addr-list">
              <ul>
                <li v-for="(item, index) in addressList" :key="index" :class="{ 'check': checkIndex === index }" @click="checkIndex = index;selectAddressId = item.addressId">
                  ...
                  <div class="addr-opration addr-default" v-if="item.isDefault">此为默认地址</div>
                </li>
                <li class="addr-new">
          ...
          <div class="next-btn-wrap">
            <a class="btn btn--m btn-disabled" v-if="!selectAddressId" >下一步</a>
            <router-link class="btn btn--m btn--red" :to="{ path: 'orderConfirm', query: { 'addressId': selectAddressId } }" v-else>下一步</router-link>
          </div>
        </div>
      </div>
    </div>
        ...
        this.$http.get('/users/addressList').then(result=>{
          ...
          if(res.status === 200){
            ...
            this.addressList.forEach(item=>{
              if (item.isDefault === true) this.selectAddressId = item.addressId;
            });
          } else {
           alert(res.msg);
          }
